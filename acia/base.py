...
class ImageSequenceSource:
    ...
    def _repr_html_(self):
        ...
        try:
            ...
            num_frames = len(self) if hasattr(self, "__len__") else 1
            num_channels = 1
        ...
        def render_image(frame_idx, active_channels):
            ...
            with output:
                ...
                try:
                    ...
                    if num_channels > 1 and len(active_channels) > 0:
                        ...
                        active_indices = [i for i, active in enumerate(active_channels) if active]
                        ...
                        if len(active_indices) == 0:
                            ...
                            if len(channel_data.shape) == 2:
                                image_data = np.repeat(channel_data[:, :, np.newaxis], 3, axis=-1)
                            else:
                                image_data = channel_data
                        else:
                            # Multiple channels - combine them
                            # For now, we'll overlay them as RGB if 3 channels, otherwise grayscale blend
                            combined = None
                            for idx in active_indices[:3]:  # Take at most first 3 channels for RGB
                                ...
                                if combined is None:
                                    if len(channel_data.shape) == 2:
                                        ...
                                        combined = np.zeros_like(channel_data, dtype=np.uint8)
                    ...
                    display(HTML(f"<p style='color: red;'>Error displaying frame: {e}</p>"))
