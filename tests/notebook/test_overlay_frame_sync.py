...
class TestOverlayFrameSynchronization(unittest.TestCase):
    ...
    def test_overlay_time_iterator_single_frame(self):
        ...
        frame_overlays = list(
            overlay.time_iterator(start_frame=0, end_frame=0)
        )
    ...
    def test_overlay_time_iterator_multiple_frames(self):
        ...
        frame_overlays = list(
            overlay.time_iterator(start_frame=0, end_frame=2)
        )
    ...
    def test_overlay_frame_indices_match_iteration_order(self):
        ...
        frame_overlays = list(
            overlay.time_iterator(start_frame=1, end_frame=3)
        )
    ...
    def test_overlay_frame_sync_with_slider_navigation(self):
        ...
        for frame_idx, expected_count in zip(test_frames, expected_contour_counts):
            frame_overlay = next(
                overlay.time_iterator(start_frame=frame_idx, end_frame=frame_idx)
            )
    ...
    def test_overlay_sparse_frames(self):
        ...
        frame_overlays = list(
            overlay.time_iterator(start_frame=0, end_frame=5)
        )
    ...
    def test_overlay_frame_boundaries(self):
        ...
        # Get last frame
        last_frame_overlay = next(
            overlay.time_iterator(start_frame=4, end_frame=4)
        )
    ...
    def test_overlay_multiple_contours_per_frame(self):
        ...
        # Get frame 2
        frame_overlay = next(
            overlay.time_iterator(start_frame=2, end_frame=2)
        )
    ...
    def test_overlay_frame_preservation_through_iteration(self):
        ...
        frame_overlays = list(
            overlay.time_iterator(start_frame=2, end_frame=6)
        )
...
class TestFrameSyncWithRenderCallback(unittest.TestCase):
    ...
    def test_render_callback_handles_missing_overlay_frame(self):
        ...
        frame_overlay = next(
            overlay.time_iterator(start_frame=1, end_frame=1)
        )
    ...
    def test_frame_sync_rapid_navigation(self):
        ...
        for frame_idx, expected_count in zip(navigation_sequence, expected_counts):
            frame_overlay = next(
                overlay.time_iterator(start_frame=frame_idx, end_frame=frame_idx)
            )
            self.assertEqual(len(frame_overlay), expected_count)
